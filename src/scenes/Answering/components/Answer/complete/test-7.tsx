...
describe('when visible, it shows the answer', () => {
    ...
    it('displays the right answer', () => {
      ...
      const testAnswer = `This has linebreaks
        ...
        //the regular expression /\s{2,}/g identifies all the linebreaks
        const testAnswerArray = testAnswer.split(/\s{2,}/g);

        const firstLine = answer.children[1];
        const secondLine = answer.children[2];
        const thirdLine = answer.children[3];

        expect(firstLine).toHaveTextContent(testAnswerArray[0]);
        expect(secondLine).toHaveTextContent(testAnswerArray[1]);
        expect(thirdLine).toHaveTextContent(testAnswerArray[2]);
    });  
